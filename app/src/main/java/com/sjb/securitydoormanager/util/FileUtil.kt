...
object FileUtil {
     ...
     * 获取属于应用的存储路径
     */
    fun getPictureCacheDir(ctx: Context = SJBApp.context): File? {
        val filesDir = ctx.getExternalFilesDir(Environment.DIRECTORY_PICTURES)
        return filesDir
    }
     ...
     * @return String
     */
    fun getAudioCacheDir(ctx: Context = SJBApp.context): String {
        val filesDir = ctx.getExternalFilesDir(Environment.DIRECTORY_MUSIC) ?: return " "
        return filesDir.path
    }

    fun getVideoCacheDir(ctx: Context = SJBApp.context): String {
...
//        val name = System.currentTimeMillis()
        val bitmapFile = File(getPictureCacheDir()?.path, "${name}.png")
        var bos: BufferedOutputStream? = null
        kotlin.runCatching {
            bos = BufferedOutputStream(FileOutputStream(bitmapFile))
            // 不压缩
            bitmap?.compress(Bitmap.CompressFormat.PNG, 100, bos)
            bos?.flush()
        }.onSuccess {
            bos?.close()
            Logger.e("bitmap生成File完成：${bitmapFile.path}")
        }.onFailure {
            Logger.e("报错：" + it.message)
            bos?.close()
        }
        return bitmapFile
    }
     ...
     * 复制文件
     *
     * @param pathFrom 从这个路径的文件复制
     * @param pathTo 复制到这个文件
     */
     ...
     * @return
     */
    fun writeFileFromIS(inputStream: InputStream?, os: OutputStream): Boolean {
    ...
    fun getFileByPath(filePath: String?): File? =
        if (StringUtil.isEmpty(filePath)) null else File(filePath)
    ...
    //创建文件，如果文件已存在则删除重新创建
    fun createFileAfterDeleteOldFile(filePath: String?) =
        createFileAfterDeleteOldFile(getFileByPath(filePath))
    ...
    fun renameFile(filePath: String?, newName: String) =
        renameFile(getFileByPath(filePath), newName)
    ...
    fun copyFile(srcFilePath: String?, destFilePath: String?) =
        copyFile(getFileByPath(srcFilePath), getFileByPath(destFilePath))

    //复制文件
    fun copyFile(srcFile: File?, destFile: File?): Boolean =
        copyOrMoveFile(srcFile, destFile, false)

    //移动文件
    fun moveFile(srcFilePath: String?, destFilePath: String?) =
        moveFile(getFileByPath(srcFilePath), getFileByPath(destFilePath))
    ...
    fun listFilesInDirectoryEndWithSuffix(
        ...
        listFilesInDirectoryEndWithSuffix(
    ...
    fun listFilesInDirectoryEndWithSuffix(
        ...
        listFilesInDirectoryWithFilter(dir, isIncludeSubDir, FilenameFilter { _, name ->
    ...
    fun listFilesInDirectoryWithFilter(
        dirPath: String?,
        isIncludeSubDir: Boolean,
